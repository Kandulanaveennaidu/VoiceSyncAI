...
export default function Header() {
  ...
  const navLinkTextColor = isScrolled
          ...
          <nav className="hidden md:flex space-x-6 items-center">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`text-sm font-medium ${navLinkTextColor} transition-colors`}
              >
                {link.label}
              </Link>
            ))}
            ...
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              ...
              >
                <div className="flex flex-col space-y-6">
                  ...
                  >
                    <motion.div animate={iconAnimation}>
                      <Waves className="h-8 w-8 text-blue-700" />{" "}
                      {/* Consistent color for mobile menu icon */}
                    </motion.div>
                    <span className="text-2xl font-bold logo-text-voicesync-gradient animate-filter-glow-voicesync">
                      VoiceSync AI
                    </span>
